...
Bookbar is a DSP component for **foobar2000** (64-bit), designed for listening to
...
The ready-to-use component (`foo_bookbar.dll`, Windows 64-bit) is published as a
...
Copy `foo_bookbar.dll` into your foobar2000 `components` folder (or use
**File → Preferences → Components → Install…** and pick the DLL), restart
foobar2000, then add **Bookbar** in **Preferences → Playback → DSP Manager**.

Requires **foobar2000 v2.0 or newer (64-bit)**.
...
REM 4) build the component
_build_plugin.bat
```

Output: `build\foo_bookbar.dll` (64-bit).
...
Bookbar to komponent DSP dla **foobara2000** (64-bit), zaprojektowany z myślą
...
Gotowy komponent (`foo_bookbar.dll`, Windows 64-bit) jest publikowany jako
...
Skopiuj `foo_bookbar.dll` do katalogu `components` foobara2000 (albo użyj
**Plik → Preferencje → Components → Install…** i wskaż plik DLL), zrestartuj
foobara2000, a następnie dodaj **Bookbar** w
**Preferencje → Playback → DSP Manager**.

Wymaga **foobara2000 v2.0 lub nowszego (64-bit)**.
...
REM 4) zbuduj komponent
_build_plugin.bat
```

Wynik: `build\foo_bookbar.dll` (64-bit).
